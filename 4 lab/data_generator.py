...
class DataGenerator:
    def __init__(self, train_data: int, test_data: int):
    ...
    def __decorator_represent_data_as_tuples(func: Callable):
        def wrapper(self):
            temp_data = []
            for item in func(self):
                key = list(item.keys())[0]
    ...
    def __generate_data(self, file: str, iterations: int = 10):
    ...
    @staticmethod
    def __get_random_positions_to_change(length: int = 35, amount: int = 1):
